<?php
/*
Plugin Name: Zip US Gateway for WooCommerce
Description: Use Zip as a payment gateway for WooCommerce.
Author: Zip Co US, Inc.
...
define( 'QUADPAY_WC_VERSION', '1.9.0' );
define( 'QUADPAY_WC_PLUGIN_URL', plugin_dir_url( __FILE__ ) );
define( 'QUADPAY_WC_PLUGIN_PATH', plugin_dir_path( __FILE__ ) );
...
function quadpay_wc_gateway() {
	...
	function quadpay_woocommerce_check_order_payment_status( $order ) {
		...
		$quadpay_order_token = $order->get_meta('_quadpay_order_token', true);
...
/**
 * Register blocks support for new checkout
 */
function quadpay_wc_blocks() {
	if ( class_exists( 'Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType' ) ) {
		require_once __DIR__ . '/includes/class-quadpay-blocks.php';
		add_action(
			'woocommerce_blocks_payment_method_type_registration',
			function ( Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry ) {
				$payment_method_registry->register( new Quadpay_WC_Blocks() );
			}
		);
	}
}
add_action( 'woocommerce_blocks_loaded', 'quadpay_wc_blocks' );

/**
 * Declare support for WooCommerce HPOS feature
 */
function quadpay_wc_declare_feature_compatibility() {
...
add_action( 'before_woocommerce_init', 'quadpay_wc_declare_feature_compatibility' );
